%% PM7 Transducer Data Analysis - All Pump Tests
% Shows drawdown and pressure profiles for all three pump tests at PM7
% Transducer data only - no DAS strain analysis
...
trans01a_z1_file = fullfile(data_dir, 'Transducer Data', 'a', 'VuSitu_2023-11-07_08-00-00_PM7_Log_PM7_1_SDT_PT-01a.csv');
trans01a_z2_file = fullfile(data_dir, 'Transducer Data', 'a', 'VuSitu_2023-11-07_08-00-00_PM7_Log_PM7_2_SDT_PT-01a.csv');
trans01a_z3_file = fullfile(data_dir, 'Transducer Data', 'a', 'VuSitu_2023-11-07_08-00-00_PM7_Log_PM7_3_SDT_PT-01a.csv');
trans01a_z4_file = fullfile(data_dir, 'Transducer Data', 'a', 'VuSitu_2023-11-07_08-00-00_PM7_Log_PM7_4_SDT_PT-01a.csv');
trans01a_z5_file = fullfile(data_dir, 'Transducer Data', 'a', 'VuSitu_2023-11-07_08-00-00_PM7_Log_PM7_5_SDT_PT-01a.csv');

% PM7 B files (Oct 31, 2023)
trans01b_z1_file = fullfile(data_dir, 'Transducer Data', 'b', 'VuSitu_2023-10-31_08-00-00_PM7_Log_PM7_1_SDT_PT-01b.csv');
trans01b_z2_file = fullfile(data_dir, 'Transducer Data', 'b', 'VuSitu_2023-10-31_08-00-00_PM7_Log_PM7_2_SDT_PT-01b.csv');
trans01b_z3_file = fullfile(data_dir, 'Transducer Data', 'b', 'VuSitu_2023-10-31_08-00-00_PM7_Log_PM7_3_SDT_PT-01b.csv');
trans01b_z4_file = fullfile(data_dir, 'Transducer Data', 'b', 'VuSitu_2023-10-31_08-00-00_PM7_Log_PM7_4_SDT_PT-01b.csv');
trans01b_z5_file = fullfile(data_dir, 'Transducer Data', 'b', 'VuSitu_2023-10-31_08-00-00_PM7_Log_PM7_5_SDT_PT-01b.csv');

% PM7 C files (Oct 24, 2023) - Zone 1 removed due to incomplete data, all zones use "SDT"
trans01c_z2_file = fullfile(data_dir, 'Transducer Data', 'c', 'VuSitu_2023-10-24_08-00-00_PM7_Log_PM7_2_SDT_PT-01c.csv');
trans01c_z3_file = fullfile(data_dir, 'Transducer Data', 'c', 'VuSitu_2023-10-24_08-00-00_PM7_Log_PM7_3_SDT_PT-01c.csv');
trans01c_z4_file = fullfile(data_dir, 'Transducer Data', 'c', 'VuSitu_2023-10-24_08-00-00_PM7_Log_PM7_4_SDT_PT-01c.csv');
trans01c_z5_file = fullfile(data_dir, 'Transducer Data', 'c', 'VuSitu_2023-10-24_08-00-00_PM7_Log_PM7_5_SDT_PT-01c.csv');

% Check all transducer files exist
required_files = {trans01a_z1_file, trans01a_z2_file, trans01a_z3_file, trans01a_z4_file, trans01a_z5_file, ...
...
fprintf('✓ All transducer data files found\n');

%% 2. Function to load and process transducer data
...
%% 3. Load all transducer data
fprintf('\n=== LOADING ALL TRANSDUCER DATA ===\n');

% Load all 5 zones for PM7 A
fprintf('Loading PM7 A transducer data:\n');
...
fprintf('\nLoading PM7 C transducer data (zones 2-5):\n');
...
%% 4. Apply time filtering to standardize datasets
...
[trans_time_01b_z4_filt, drawdown_01b_z4_filt] = filter_by_duration(trans_time_01b_z4, drawdown_01b_z4, start_time_01b, 5, 25, 0);
...
%% 5. Apply smoothing to transducer data
...
%% 6. Create drawdown comparison plots
fprintf('\n=== CREATING DRAWDOWN COMPARISON PLOTS ===\n');

% Figure settings
figure('Position', [100, 100, 1200, 900]);

% Define colors and depths for each zone
...
% Plot 1: PM7 A all 5 zones
subplot(3,1,1)
hold on
if ~isempty(drawdown_01a_z1_smooth)
    plot(trans_time_01a_z1_filt, drawdown_01a_z1_smooth, 'Color', zone_colors(1,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01a_z2_smooth)
    plot(trans_time_01a_z2_filt, drawdown_01a_z2_smooth, 'Color', zone_colors(2,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01a_z3_smooth)
    plot(trans_time_01a_z3_filt, drawdown_01a_z3_smooth, 'Color', zone_colors(3,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01a_z4_smooth)
    plot(trans_time_01a_z4_filt, drawdown_01a_z4_smooth, 'Color', zone_colors(4,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01a_z5_smooth)
    plot(trans_time_01a_z5_filt, drawdown_01a_z5_smooth, 'Color', zone_colors(5,:), 'LineWidth', 2, 'LineStyle', '-');
end
hold off

ylabel('Drawdown (ft)')
title('PM7 A: All Zone Drawdowns')
grid on
legend({['Zone 1 (' zone_depths{1} ')'], ['Zone 2 (' zone_depths{2} ')'], ...
...
for i = 2:length(pump_times_01a)
    ...
    if i <= length(pump_rates_01a)-1
        text(pump_times_01a(i), max(ylim), sprintf('%d GPM', pump_rates_01a(i)), ...
             'HorizontalAlignment', 'center', 'FontSize', 9, 'FontWeight', 'bold', ...
             'BackgroundColor', 'white', 'EdgeColor', 'black');
    else
        text(pump_times_01a(i), max(ylim), '0 GPM', 'HorizontalAlignment', 'center', ...
             'FontSize', 9, 'FontWeight', 'bold', 'BackgroundColor', 'white', 'EdgeColor', 'black');
    end
end
hold off

% Plot 2: PM7 B all 5 zones
subplot(3,1,2)
hold on
if ~isempty(drawdown_01b_z1_smooth)
    plot(trans_time_01b_z1_filt, drawdown_01b_z1_smooth, 'Color', zone_colors(1,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01b_z2_smooth)
    plot(trans_time_01b_z2_filt, drawdown_01b_z2_smooth, 'Color', zone_colors(2,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01b_z3_smooth)
    plot(trans_time_01b_z3_filt, drawdown_01b_z3_smooth, 'Color', zone_colors(3,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01b_z4_smooth)
    plot(trans_time_01b_z4_filt, drawdown_01b_z4_smooth, 'Color', zone_colors(4,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01b_z5_smooth)
    plot(trans_time_01b_z5_filt, drawdown_01b_z5_smooth, 'Color', zone_colors(5,:), 'LineWidth', 2, 'LineStyle', '-');
end
hold off

ylabel('Drawdown (ft)')
title('PM7 B: All Zone Drawdowns')
grid on
legend({['Zone 1 (' zone_depths{1} ')'], ['Zone 2 (' zone_depths{2} ')'], ...
        ['Zone 3 (' zone_depths{3} ')'], ['Zone 4 (' zone_depths{4} ')'], ['Zone 5 (' zone_depths{5} ')']}, ...
       'Location', 'eastoutside', 'FontSize', 8)
...
for i = 2:length(pump_times_01b)
    ...
    if i <= length(pump_rates_01b)-1
        text(pump_times_01b(i), max(ylim), sprintf('%d GPM', pump_rates_01b(i)), ...
             'HorizontalAlignment', 'center', 'FontSize', 9, 'FontWeight', 'bold', ...
             'BackgroundColor', 'white', 'EdgeColor', 'black');
    else
        text(pump_times_01b(i), max(ylim), '0 GPM', 'HorizontalAlignment', 'center', ...
             'FontSize', 9, 'FontWeight', 'bold', 'BackgroundColor', 'white', 'EdgeColor', 'black');
    end
end
hold off

% Plot 3: PM7 C zones 2-5 (Zone 1 removed due to incomplete data)
subplot(3,1,3)
hold on
if ~isempty(drawdown_01c_z2_smooth)
    plot(trans_time_01c_z2_filt, drawdown_01c_z2_smooth, 'Color', zone_colors(2,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01c_z3_smooth)
    plot(trans_time_01c_z3_filt, drawdown_01c_z3_smooth, 'Color', zone_colors(3,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01c_z4_smooth)
    plot(trans_time_01c_z4_filt, drawdown_01c_z4_smooth, 'Color', zone_colors(4,:), 'LineWidth', 2, 'LineStyle', '-');
end
if ~isempty(drawdown_01c_z5_smooth)
    plot(trans_time_01c_z5_filt, drawdown_01c_z5_smooth, 'Color', zone_colors(5,:), 'LineWidth', 2, 'LineStyle', '-');
end
hold off

ylabel('Drawdown (ft)')
title('PM7 C: Zone Drawdowns (Zones 2-5)')
grid on
legend({['Zone 2 (' zone_depths{2} ')'], ['Zone 3 (' zone_depths{3} ')'], ...
        ['Zone 4 (' zone_depths{4} ')'], ['Zone 5 (' zone_depths{5} ')']}, ...
       'Location', 'eastoutside', 'FontSize', 8)
...
for i = 2:length(pump_times_01c)
    ...
    if i <= length(pump_rates_01c)-1
        text(pump_times_01c(i), max(ylim), sprintf('%d GPM', pump_rates_01c(i)), ...
             'HorizontalAlignment', 'center', 'FontSize', 9, 'FontWeight', 'bold', ...
             'BackgroundColor', 'white', 'EdgeColor', 'black');
    else
        text(pump_times_01c(i), max(ylim), '0 GPM', 'HorizontalAlignment', 'center', ...
             'FontSize', 9, 'FontWeight', 'bold', 'BackgroundColor', 'white', 'EdgeColor', 'black');
    end
end
hold off
...
% Overall title
sgtitle('PM7 Well Pump Test Comparisons: Transducer Drawdown Data Only', 'FontSize', 14)

fprintf('✓ Created drawdown comparison plots\n');
fprintf('✓ PM7 Transducer Analysis completed successfully!\n');