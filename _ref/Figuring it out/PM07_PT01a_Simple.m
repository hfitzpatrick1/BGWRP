...
% Note time range of data from file labels - PT01a November 7, 2023
...
% For PT01a, start integration at a logical time point
% Start integration 30 minutes before recovery (20:15 UTC)
time_before_recovery = datetime(2023,11,7,20,15,00,00,'TimeZone','UTC');
integration_start_time = time_before_recovery;
integration_start = max(1, round(seconds(integration_start_time - DASStart)));
if integration_start > data_length
    integration_start = max(1, round(data_length * 0.1));  % Fallback to 10% if calculated time is beyond data
end
...
colormap('jet')
colorbar
title('Raw Data - PT01a')
xlabel('Sample')
ylabel('Channel Number')
%% Plot Displacement Rate

% PT01a pump test timing - November 7, 2023
Start50= datetime(2023,11,7,20,44,00,00,'TimeZone','UTC'); 
...
% Focus on 5-minute recovery window: 20:44-20:49 UTC (authoritative)
