% Extract PT-01b displacement rate from toolkit output and export to LAS
% This uses the exact data that appears in Figure 102 (the black line)

clear; clc;

%% Load the processed DAS data from the toolkit
data_dir = 'C:\Coding\BGWRP\data\_BATCH\_active\PT01b_Recovery_short\_das';
das_file = fullfile(data_dir, 'Dataset_PT01b_Recovery_short_1Hz.mat');

fprintf('Loading DAS data: %s\n', das_file);
load(das_file);

%% Apply the exact same processing as the toolkit
% From analyze_das_data.m for PT01b
...
% Calculate depth (matching PT_01b_CC.m)
...
timing_file = 'C:\Coding\BGWRP\data\_BATCH\_active\PT01b_Recovery_short\_das_timing\get_timing_PT01b_Recovery_short.m';
addpath(fileparts(timing_file));
timing_config = get_timing_PT01b_Recovery_short();
...
analysis_window = [datetime(2023, 10, 31, 19, 29, 0, 'TimeZone', 'UTC'), ...
                   datetime(2023, 10, 31, 19, 32, 0, 'TimeZone', 'UTC')];
...
fprintf(fid, 'DAS_MEAN.NM : DAS Mean Displacement Rate (Recovery 19:29:00-19:32:00)\n');
fprintf(fid, '\n');
...
title('PT-01b Mean Displacement Rate (from Toolkit)', 'FontSize', 14);
grid on;
