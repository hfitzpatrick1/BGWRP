%% ROI Analysis Script for PT01b 100 Hz Data
...
% Detect which PT01b dataset was processed
console_log('DEBUG: Checking for PT01b datasets in das_results...\n');
all_datasets = fieldnames(das_results);
console_log('DEBUG: Available datasets: %s\n', strjoin(all_datasets, ', '));

if isfield(das_results, 'PT01b_Recovery_100')
    dataset_name = 'PT01b_Recovery_100';
    console_log('Using 100 Hz dataset: PT01b_Recovery_100\n');
elseif isfield(das_results, 'PT01b_Recovery_short')
    dataset_name = 'PT01b_Recovery_short';
    console_log('Using 1 Hz dataset: PT01b_Recovery_short\n');
else
    error('No PT01b dataset found in das_results. Available datasets: %s', strjoin(all_datasets, ', '));
end
...
lr_config = struct();
lr_config.zone = 'z4';
lr_config.depth_range_ft = [350, 400];  % PT01b screened interval (107-122 m)
lr_config.timing_correction_sec = -12;  % Head shifted RIGHT 12s to align with DAS
