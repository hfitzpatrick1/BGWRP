...
console_log('\n');
console_log('╔════════════════════════════════════════════════════════════════╗\n');
console_log('║  AMPLITUDE-BASED STORAGE CALCULATION (Single Channel Method)  ║\n');
console_log('╚════════════════════════════════════════════════════════════════╝\n');
...
console_log('\nConfiguration:\n');
console_log('  Target depth: %.1f ft\n', target_depth_ft);
console_log('  Zone: %s\n', zone_name);
console_log('  Time period for rate: %.0f seconds\n', time_period);
console_log('  Alpha (Biot-Willis): %.2f\n', alpha);
console_log('  Gamma (water): %.0f N/m³\n', gamma);
...
console_log('\n=== CHANNEL SELECTION ===\n');
console_log('  Requested depth: %.1f ft\n', target_depth_ft);
console_log('  Closest channel: %d\n', ch_idx);
console_log('  Actual depth: %.1f ft\n', actual_depth);
...
console_log('  Max: %.4e nm/s\n', dz_max);
console_log('  Min: %.4e nm/s\n', dz_min);
console_log('  Amplitude (max-min): %.4e nm/s\n', dz_amplitude);
...
console_log('  (Expected range: 1e-10 to 1e-11 for unconsolidated sediment)\n');
...
console_log('\n=== HEAD RATE ===\n');
console_log('  Drawdown max: %.4f ft\n', drawdown_max);
console_log('  Drawdown min: %.4f ft\n', drawdown_min);
console_log('  Drawdown range: %.4f ft\n', drawdown_range);
console_log('  Time period: %.0f seconds\n', time_period);
console_log('  Head rate: %.4e ft/s\n', head_rate);
console_log('  (= %.4e ft/min)\n', head_rate * 60);

%% Calculate slope
slope = strain_rate_amplitude / head_rate;  % (1/s) / (ft/s) = 1/ft

console_log('\n=== SLOPE ===\n');
console_log('  Slope = strain_rate / head_rate\n');
console_log('  Slope: %.4e (1/s)/(ft/s)\n', slope);

% Convert to SI units
slope_SI = slope / 0.3048;  % Convert from 1/ft to 1/m

console_log('  Slope (SI): %.4e 1/m\n', slope_SI);
...
console_log('  Sε = (α / γ) × slope\n');
console_log('  Sε = (%.2f / %.0f) × %.4e\n', alpha, gamma, slope_SI);
console_log('  Sε (constrained): %.4e 1/Pa\n', Se);

% Ss = Se * gamma
Ss = Se * gamma;  % 1/m

console_log('\n  Ss = Sε × γ\n');
console_log('  Ss = %.4e × %.0f\n', Se, gamma);
console_log('  Ss (specific storage): %.4e 1/m\n', Ss);

%% Compare to expected ranges
console_log('\n=== QUALITY CHECK ===\n');
console_log('  Expected Ss for unconsolidated alluvium: 1e-07 to 1e-04 1/m\n');
console_log('  Your Ss: %.2e 1/m\n', Ss);

if Ss >= 1e-07 && Ss <= 1e-04
    console_log('  ✓ GOOD: Within expected range for unconsolidated sediment\n');
elseif Ss < 1e-07
    console_log('  ⚠ WARNING: Too low - typical of consolidated rock\n');
else
    console_log('  ⚠ WARNING: Too high - check calculation\n');
end
...
console_log('\n');
console_log('╔═══════════════════════════════════════╗\n');
console_log('║  AMPLITUDE CALCULATION COMPLETE!      ║\n');
console_log('╚═══════════════════════════════════════╝\n');
console_log('\n');
