...
%% Generate Standalone Figures for Publication
if config.show_plots
    fprintf('\n=== GENERATING STANDALONE FIGURES ===\n');
    
    % STANDALONE FIGURE (a): Linear Regression Scatter
    fig_a = figure(200); clf;
    set(fig_a, 'Position', [100 100 800 700], 'Color', 'white');
    set(fig_a, 'Name', 'Figure (a) Linear Regression');
    
    scatter(head_rate_clean, strain_clean, 35, 'b', 'filled', 'MarkerFaceAlpha', 0.7);
    hold on;
    head_rate_range = linspace(min(head_rate_clean), max(head_rate_clean), 100);
    plot(head_rate_range, intercept + slope * head_rate_range, 'r-', 'LineWidth', 3);
    hold off;
    
    xlabel('Drawdown Rate (m/s)', 'FontSize', 14, 'FontWeight', 'bold');
    ylabel('Strain Rate (1/s)', 'FontSize', 14, 'FontWeight', 'bold');
    title(sprintf('Linear Regression: R = %.3f, R^2 = %.3f', R_corr, R_squared), ...
        'FontSize', 16, 'FontWeight', 'bold');
    grid on;
    legend({'Data', sprintf('Fit: y = %.2e*x + %.2e', slope, intercept)}, ...
    ...
    text(0.05, 0.95, text_str, 'Units', 'normalized', ...
        ...
        'EdgeColor', 'black', 'LineWidth', 1.5, 'FontSize', 11);
    
    fprintf('  ✓ Figure 200 (a): Linear Regression Scatter\n');
    
    % STANDALONE FIGURE (b): Time Series
    fig_b = figure(201); clf;
    set(fig_b, 'Position', [150 150 1200 600], 'Color', 'white');
    set(fig_b, 'Name', 'Figure (b) Time Series');
    ...
    yyaxis left;
    plot(time_clean, head_rate_clean / head_scale, ...
        'Color', [0.4660 0.6740 0.1880], 'LineWidth', 2.5);
    ylabel(sprintf('Drawdown Rate (m/s) ×10^{%d}', round(log10(head_scale))), ...
        'FontSize', 14, 'FontWeight', 'bold');
    ax = gca;
    ax.YColor = [0.4660 0.6740 0.1880];
    
    % Right axis: Strain Rate
    yyaxis right;
    plot(time_clean, strain_clean / strain_scale, ...
    ...
    title(sprintf('Time Series (%ds correction) - Depth 76-107 m', config.timing_correction_sec), ...
        'FontSize', 16, 'FontWeight', 'bold');
    grid on;
    set(gca, 'FontSize', 12, 'LineWidth', 1.5);
    legend({'Drawdown Rate', 'Strain Rate'}, 'Location', 'best', 'FontSize', 12);
    datetick('x', 'HH:MM:SS', 'keepticks');
    
    fprintf('  ✓ Figure 201 (b): Time Series\n');
    fprintf('\n  Figures 200 & 201 are standalone versions ready for your poster!\n');
end
