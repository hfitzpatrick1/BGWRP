...
console_log('If displacement = %.2f nm/s:\n', displacement_nm_per_s);
...
console_log('  If in nm/s: 0.01 to 1.0 nm/s\n');
console_log('  If in µm/s: 0.01 to 1.0 µm/s (would appear as 0.01 to 1.0 if misread as nm/s)\n');
console_log('  If in m/s: 1e-11 to 1e-9 m/s\n\n');

console_log('QUESTION: What are typical displacement rate values in your data?\n');
console_log('  - If 0.1-1.0 range: likely nm/s or µm/s\n');
console_log('  - If 1e-9 to 1e-11 range: likely m/s\n');
console_log('  - If 0.1-1.0 and you get 1000× error: probably µm/s, not nm/s!\n');
