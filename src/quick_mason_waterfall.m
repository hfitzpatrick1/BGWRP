function quick_mason_waterfall()
    ...
    fprintf('=== Quick Mason TDMS Waterfall Visualization ===\n\n');
    ...
    fprintf('Loading Mason''s processed data...\n');
    fprintf('File: %s\n', mason_file);
    ...
    if isfield(loaded, 'Data')
        data = loaded.Data;
        fprintf('Loaded Data variable: [%d x %d]\n', size(data, 1), size(data, 2));
    else
    ...
    if nrows > ncols
        ...
        fprintf('Detected format: [time x channels] = [%d x %d]\n', time_samples, total_channels);
        ...
        fprintf('Detected format: [channels x time] = [%d x %d], transposing...\n', total_channels, time_samples);
    ...
    fprintf('Using data subset: [%d x %d] (time x channels)\n', size(channel_data, 1), size(channel_data, 2));
    ...
    fprintf('Figure window should be displayed.\n');
