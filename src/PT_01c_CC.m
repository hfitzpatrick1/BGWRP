%% DAS Step Test & Recovery Analysis – PT-01c (Clean Signal Focus with Common Mode Removal)
% Clean visualization focused on exposing both step test pumping and recovery signal patterns
% Step test data from Oct 24, 2023
% Uses common mode removal techniques to enhance signal quality
% Well Screen: 260-310 ft

clear; clc; close all
fprintf('=== PT-01c Common Mode Removal Analysis Started ===\n');
fprintf('Script: PT_01c_CC.m\n');
fprintf('Date: Oct 24, 2023 Step Test\n');
fprintf('Well Screen: 260-310 ft\n\n');
...
%% 4a. Acoustic Resonance Filtering (added for wells without foam)
...
fprintf('Acoustic filtering complete - should improve signal quality\n');

%% 2. Depth control (PT01c parameters)
fprintf('Setting up depth control parameters...\n');
...
% Define pumping zone for analysis (PT01c well screening zone)
pump_zone_top = 260;    % ft - well screened zone
pump_zone_bot = 310;    % ft - well screened zone  
pump_zone_center = 285; % ft
...
% Focus on the well casing region (PT01c uses 0-665 ft range)
...
%% 5. Integration and baseline (PT01c approach)
fprintf('\n--- Integration and Baseline ---\n');
intdata = cumtrapz(data_cleaned, 1);
idata_roi = intdata(:, roi);

% Use PT01c baseline approach: 120-s flat pre-pump pad
pump_on  = datetime(2023,10,24,15,18,0,'TimeZone','UTC');
...
% Define recovery period for PT01c (pumps shut off at 19:15)
% PT01c pump schedule:
% 15:18 - 50 GPM, 16:15 - 80 GPM, 17:15 - 110 GPM, 18:15 - 148 GPM, 19:15 - 0 GPM (recovery)
recovery_start = datetime(2023,10,24,19,15,0,'TimeZone','UTC');  % Pumps shut off
recovery_end = datetime(2023,10,24,20,15,0,'TimeZone','UTC');

% Pumping period definition (for additional pumping analysis)
% PT01c step test pumping period
pumping_start = datetime(2023,10,24,15,18,0,'TimeZone','UTC');  % First step begins
...
ylim([0 well_depth_ft])  % PT01c uses 0-665 ft range
xlim([datenum(recovery_start), datenum(recovery_end)])

% Clean time axis
datetick('x','HH:MM','keeplimits')
title('Aquifer Recovery Signal – PT-01c Step Test (Pumps Off 19:15)', 'FontSize', 14)
...
fprintf('✅ Figure 1 created: Full recovery heatmap\n');

%% 6b. Focused well screen analysis (260-310 ft)  
fprintf('\n--- Focused Well Screen Analysis ---\n');

% Extract pumping zone data with extended bounds for figure 4
pump_zone_buffer_top = 200;  % Extended view to 200 ft for better context
pump_zone_buffer_bot = pump_zone_bot + 20;  % 330 ft
...
if ~exist(las_file, 'file')
    ...
    try
        ...
        % Filter to pumping zone plus buffer (200-330 ft) for focused plot
        pump_zone_las_mask = las_depth >= 200 & las_depth <= 330;
        ...
        % Filter to full well depth range (0-665 ft) for full plot
        full_well_las_mask = las_depth >= 0 & las_depth <= 665;
...
if ~isempty(gr_data)
    ...
    hold on
    % Extended view boundary
    yline(pump_zone_buffer_top, 'r--', 'LineWidth', 2, 'Label', 'Extended View (200 ft)')
    
    % 20 feet below pumping zone (330 ft)
    yline(pump_zone_buffer_bot, 'r--', 'LineWidth', 2, 'Label', '20 ft Below Well Screen')
    
    % Pumping zone boundaries (well screen)
    yline(pump_zone_top, 'k-', 'LineWidth', 3, 'Label', 'Well Screen Top (260 ft)')
    yline(pump_zone_bot, 'k-', 'LineWidth', 3, 'Label', 'Well Screen Bot (310 ft)')
    ...
    if water_table_depth_ft >= pump_zone_buffer_top && water_table_depth_ft <= pump_zone_buffer_bot
        yline(water_table_depth_ft, 'w--', 'LineWidth', 2, 'Label', 'Water Table (89 ft)')
    end
...
datetick('x','HH:MM','keeplimits')
title('Recovery Signal (200-330 ft)', 'FontSize', 12)
xlabel('Time (UTC)', 'FontSize', 12)
...
if ~isempty(gr_data)
    sgtitle('PT-01c Recovery Signal with Gamma Ray Log (200-330 ft)', 'FontSize', 14)
else
    title('PT-01c Recovery Signal (200-330 ft)', 'FontSize', 14)
end

% Force figure to be visible and on top
set(gcf, 'Visible', 'on');
figure(4);  % Bring to front
fprintf('✅ Figure 4 created: Well screen focused analysis\n');
...
if ~isempty(gr_data_full)
    ...
    % Add overall title for the figure
    sgtitle('PT-01c Full Recovery Signal with Gamma Ray Log', 'FontSize', 14)
    
    % Force figure to be visible and on top
    set(gcf, 'Visible', 'on');
    figure(6);  % Bring to front
    fprintf('✅ Figure 6 created: Full recovery with gamma ray overlay\n');
end
...
if isempty(dstrain_pumping)
    ...
    if ~isempty(gr_data_full)
        ...
        % Add overall title for the figure
        sgtitle('Full Step Test Signal with Gamma Ray Log – PT-01c', 'FontSize', 14)
        ...
        xlim([datenum(pumping_start), datenum(pumping_end)])
        ylabel('Depth (ft)', 'FontSize', 12)
        title('Aquifer Step Test Signal – PT-01c', 'FontSize', 14)
    end
    ...
    fprintf('✅ Figure 7 created: Full step test heatmap\n');
    ...
    if ~isempty(gr_data)
        ...
        % Add overall title for the figure
        sgtitle('Step Test Well Screen Signal with Gamma Ray Log (200-330 ft) – PT-01c', 'FontSize', 14)
        ...
        xlim([datenum(pumping_start), datenum(pumping_end)])
        ylabel('Depth (ft)', 'FontSize', 12)
        title('Step Test Signal (200-330 ft) – PT-01c', 'FontSize', 14)
    ...
    % Force figure to be visible and on top
    set(gcf, 'Visible', 'on');
    figure(8);  % Bring to front
    fprintf('✅ Figure 8 created: Well screen step test analysis\n');
    ...
    if sum(t_early_pumping) > 0 && sum(t_late_pumping) > 0
        ...
        sgtitle('Step Test Analysis – PT-01c', 'FontSize', 14)
        
        % Define zone masks for step test analysis
        above_pump = final_depth_ft >= 0 & final_depth_ft < pump_zone_top;
        ...
        % Force figure to be visible and on top
        set(gcf, 'Visible', 'on');
        figure(9);  % Bring to front
        fprintf('✅ Figure 9 created: Step test progression analysis\n');
        ...
        fprintf('\nPT-01c Step Test Schedule:\n');
        fprintf('  15:18 - 50 GPM (%.1f hr)\n', hours(datetime(2023,10,24,16,15,0,'TimeZone','UTC') - pumping_start));
        fprintf('  16:15 - 80 GPM (%.1f hr)\n', hours(datetime(2023,10,24,17,15,0,'TimeZone','UTC') - datetime(2023,10,24,16,15,0,'TimeZone','UTC')));
        fprintf('  17:15 - 110 GPM (%.1f hr)\n', hours(datetime(2023,10,24,18,15,0,'TimeZone','UTC') - datetime(2023,10,24,17,15,0,'TimeZone','UTC')));
        fprintf('  18:15 - 148 GPM (%.1f hr)\n', hours(pumping_end - datetime(2023,10,24,18,15,0,'TimeZone','UTC')));
        fprintf('  19:15 - 0 GPM (recovery begins)\n');
...
%% 7. Recovery period comparison analysis (adapted for PT01c)
fprintf('\n--- Recovery Period Analysis ---\n');

% Define recovery periods for PT01c step test
t_early_recovery = Tdas >= datetime(2023,10,24,19,15,0,'TimeZone','UTC') & ...
                   Tdas <  datetime(2023,10,24,19,30,0,'TimeZone','UTC');
t_late_recovery = Tdas >= datetime(2023,10,24,20,0,0,'TimeZone','UTC') & ...
                  Tdas <  datetime(2023,10,24,20,15,0,'TimeZone','UTC');
...
fprintf('Early recovery period: 19:15-19:30 (%.1f minutes)\n', 15);
fprintf('Late recovery period: 20:00-20:15 (%.1f minutes)\n', 15);
...
legend('Early Recovery (19:15-19:30)', 'Late Recovery (20:00-20:15)', 'Location', 'best')
ylim([0 well_depth_ft])
...
sgtitle('Recovery Analysis – PT-01c (with CM Removal)', 'FontSize', 14)

% Force figure to be visible and on top
set(gcf, 'Visible', 'on');
figure(2);  % Bring to front
fprintf('✅ Figure 2 created: Recovery comparison profiles\n');

%% 9. Zone-based analysis (background calculations)
% Calculate zone statistics without heavy visualization (well screen 260-310 ft)
above_pump = final_depth_ft >= 0 & final_depth_ft < pump_zone_top;
pump_zone = final_depth_ft >= pump_zone_top & final_depth_ft <= pump_zone_bot;
below_pump = final_depth_ft > pump_zone_bot & final_depth_ft <= well_depth_ft;
...
xlim([datetime(2023,10,24,15,18,0,'TimeZone','UTC') ...
      datetime(2023,10,24,20,15,0,'TimeZone','UTC')])
...
fprintf('  Above well screen (0-260 ft): %.1f nε\n', above_pump_recovery);
fprintf('  Well screen zone (260-310 ft): %.1f nε\n', pump_zone_recovery);
fprintf('  Below well screen (310-665 ft): %.1f nε\n', below_pump_recovery);
...
fprintf('  - Figure 1: Full recovery heatmap (0-665 ft)\n');
fprintf('  - Figure 2: Recovery comparison profiles\n');
fprintf('  - Figure 4: Well screen focused analysis (200-330 ft)\n');
...
if exist('dstrain_pumping', 'var') && ~isempty(dstrain_pumping)
    ...
    fprintf('  - Figure 8: Well screen step test analysis (200-330 ft)\n');
    if exist('step_progression', 'var')
