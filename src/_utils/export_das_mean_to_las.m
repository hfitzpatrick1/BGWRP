...
fprintf('=== EXPORTING DAS MEAN TO LAS FORMAT ===\n');
fprintf('Dataset: %s\n', dataset_name);
fprintf('Output: %s\n', output_filename);
...
fprintf('Depth range: [%.1f, %.1f] ft (shifted down by %.0f ft)\n', min(depth_ft), max(depth_ft), depth_shift_ft);
...
fprintf('Calculating mean displacement rate by depth...\n');
...
fprintf('Mean calculated for %d depth points\n', length(mean_data));
...
fprintf(fid, 'DAS_MEAN.NM : DAS Mean Displacement Rate\n');
fprintf(fid, '\n');
...
fprintf('Mean displacement rate range: [%.3e, %.3e]\n', min(mean_data), max(mean_data));
fprintf('Mean value: %.3e nm/s\n', mean(mean_data));
