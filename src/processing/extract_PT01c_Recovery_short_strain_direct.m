...
console_log('=== EXTRACTING PT01c_Recovery_short STRAIN DATA (DIRECT) ===\n');
...
elseif isfield(loaded_data, 'data1Hz')
    ...
    console_log('  Loaded 1Hz data: [%d x %d]\n', size(data1Hz, 1), size(data1Hz, 2));
else
...
console_log('Using PT01c calibration: C1=%d, MperChan=%.3f\n', C1, MperChan);
...
console_log('Applied 5-second moving average smoothing\n');
...
console_log('✓ Full strain matrix saved to: %s\n', filename_full);
...
console_log('✓ Analysis window strain data saved to: %s\n', filename_analysis);
...
console_log('✓ Strain matrix also saved as CSV: %s\n', csv_filename);

console_log('\n=== EXTRACTION COMPLETE ===\n');
console_log('Files created for your advisor:\n');
console_log('  - %s (full dataset)\n', filename_full);
console_log('  - %s (analysis window only)\n', filename_analysis);
console_log('  - %s (CSV format)\n', csv_filename);
