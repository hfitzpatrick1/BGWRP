%% Standalone ROI Analysis Script for PT-01c
...
% Auto-detect the PT01c dataset name from das_results
% Works with either PT01c_Recovery_short (1Hz) or PT01c_Recovery_100 (100Hz)
das_fields = fieldnames(das_results);
pt01c_idx = find(startsWith(das_fields, 'PT01c_Recovery'));
if isempty(pt01c_idx)
    error('No PT01c_Recovery dataset found in das_results. Available fields: %s', strjoin(das_fields, ', '));
end
test_name = das_fields{pt01c_idx(1)};
...
lr_config.timing_correction_sec = -14;  % Head shifted RIGHT +14s to align with DAS (matches plot convention)
