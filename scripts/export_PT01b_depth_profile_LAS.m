%% Export PT01b Depth Profile to LAS file for WellCAD
% Run AFTER run_PT01b_thesis_analysis.m (needs das_results in workspace)

if ~exist('das_results', 'var')
    error('Run the thesis analysis script first to populate das_results.');
end

% Auto-detect PT01b dataset
das_fields = fieldnames(das_results);
pt01b_idx = find(startsWith(das_fields, 'PT01b_Recovery'));
if isempty(pt01b_idx)
    error('No PT01b dataset found in das_results.');
end
test_name_las = das_fields{pt01b_idx(1)};
das_data_las = das_results.(test_name_las);

% Regression window (same as run_roi_analysis_PT01b.m)
...
output_file = fullfile(output_dir, 'PT01b_DAS_Profile_100Hz.las');
